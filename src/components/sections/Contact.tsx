...
const services = [
  ...
  "Book Branding", "Book Designing", "Logo Design", "Others",
];
...
function Field({ label, icon: Icon, required, children }: {
  ...
  return (
    <div className="flex flex-col gap-1.5 sm:gap-2 min-w-0">
      <label className="flex items-center gap-1.5 text-[10.5px] sm:text-[11px] font-bold text-slate-500 uppercase tracking-widest select-none">
        <Icon className="w-3.5 h-3.5 text-[var(--accent-global)] opacity-80 shrink-0" />
        <span className="truncate">{label}</span>{required && <span className="text-[var(--accent-global)]">*</span>}
      </label>
      {children}
    </div>
  );
...
const inputCls = "w-full bg-slate-50 border border-slate-200 focus:border-[var(--accent-global)] focus:bg-white focus:ring-2 focus:ring-[var(--accent-global)]/10 rounded-xl px-3.5 py-3 sm:px-4 sm:py-3.5 text-sm outline-none text-slate-800 placeholder-slate-400 transition-all duration-200 font-medium";
...
export default function Contact() {
  ...
  return (
    <section className="border-t relative overflow-hidden bg-slate-950 border-white/[0.06] pb-0 w-full" id="contact">
      ...
      {/* Main content */}
      <div className="max-w-[1400px] mx-auto px-3.5 sm:px-6 lg:px-8 pt-16 sm:pt-20 lg:pt-24 pb-12 sm:pb-16 relative z-10 w-full min-w-0">
        <SectionHeader
          badge="CONTACT US"
        ...
        <div className="grid lg:grid-cols-[1fr_400px] gap-8 sm:gap-10 items-start w-full min-w-0">

          {/* Form Card */}
          <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl shadow-black/40 overflow-hidden w-full min-w-0 border border-slate-100">
            <div className="h-1.5 w-full bg-gradient-to-r from-[var(--accent-global)] via-purple-400 to-cyan-400" />
            <div className="p-4 sm:p-7 md:p-8 lg:p-10">
              {sent ? (
                <div className="flex flex-col items-center text-center py-12 sm:py-16 gap-5">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-gradient-to-br from-[var(--accent-global)] to-purple-500 flex items-center justify-center shadow-lg shadow-purple-500/30">
                    <CheckCircle2 className="w-8 h-8 sm:w-10 sm:h-10 text-white" />
                  </div>
                  <div>
                    <h3 className="text-xl sm:text-2xl font-bold text-slate-900 mb-2 font-sora">Message Sent! 🎉</h3>
                    <p className="text-xs sm:text-sm text-slate-500 max-w-xs mx-auto leading-relaxed">Our team will reach out within 24 hours. We&apos;re excited to work with you.</p>
                  </div>
                  {leadId && (
                ...
                <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                  ...
                  {error && (
                    <div className="flex items-start gap-3 p-3.5 sm:p-4 rounded-xl text-sm bg-red-50 border border-red-100 text-red-700">
                  ...
                  <div className="flex flex-col gap-2 sm:gap-3 min-w-0">
                    <label className="flex items-center gap-1.5 text-[10.5px] sm:text-[11px] font-bold text-slate-500 uppercase tracking-widest">
                      <span className="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 rounded-full bg-[var(--accent-global)] opacity-80 inline-block shrink-0" />
                    ...
                    <div className="grid grid-cols-2 sm:flex sm:flex-wrap gap-2 sm:gap-2.5">
                      {services.map((s) => (
                        <button key={s} type="button" onClick={() => setForm({ ...form, service: s })}
                          className={`px-2.5 py-2.5 sm:px-4 sm:py-2 rounded-xl text-[11px] sm:text-xs font-semibold border-2 transition-all duration-200 cursor-pointer flex items-center justify-center text-center leading-tight ${form.service === s ? "bg-[var(--accent-global)] border-[var(--accent-global)] text-white shadow-md shadow-purple-500/20 scale-[1.02] sm:scale-105" : "bg-slate-50 border-slate-200 text-slate-600 hover:border-[var(--accent-global)]/50 hover:text-[var(--accent-global)] hover:bg-slate-100"}`}>
                          {s}
                        </button>
                      ))}
                    </div>
                  ...
                  <div className="flex flex-col gap-1.5 sm:gap-2 min-w-0">
                    <label className="flex items-center gap-1.5 text-[10.5px] sm:text-[11px] font-bold text-slate-500 uppercase tracking-widest">
                      <span className="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 rounded-full bg-[var(--accent-global)] opacity-80 inline-block shrink-0" />
                      <span>Project Details</span> <span className="text-[var(--accent-global)]">*</span>
                    </label>
                    <textarea required rows={4} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} placeholder="Tell us about your project, goals, and timeline. The more detail, the better we can help you." className={inputCls + " resize-none leading-relaxed"} />
                  </div>
                  <button type="submit" disabled={sending}
                    className="w-full bg-gradient-to-r from-[var(--accent-global)] to-purple-600 hover:from-purple-600 hover:to-[var(--accent-global)] disabled:opacity-60 text-white font-bold text-sm h-12 sm:h-14 rounded-xl flex items-center justify-center gap-2.5 sm:gap-3 transition-all duration-300 shadow-lg shadow-purple-500/25 hover:shadow-purple-500/40 hover:-translate-y-0.5 cursor-pointer tracking-wide">
                    {sending ? (<><div className="w-4 h-4 sm:w-5 sm:h-5 rounded-full border-2 border-white/30 border-t-white animate-spin" /><span>Sending...</span></>) : (<><Send className="w-4 h-4 sm:w-5 sm:h-5" /><span>Send Message</span><ArrowRight className="w-3.5 h-3.5 sm:w-4 sm:h-4 opacity-70" /></>)}
                  </button>
                  <p className="text-center text-[10.5px] sm:text-[11px] text-slate-400 flex items-center justify-center gap-1.5">
                    <span>🔒</span>
          ...
          {/* Info Panels */}
          <div className="space-y-4 sm:space-y-5 lg:pt-2 w-full min-w-0">
            {[
              ...
              <a key={label} href={href} className="flex items-center gap-3.5 sm:gap-4 bg-slate-900 border border-white/[0.07] hover:border-[var(--accent-global)]/40 rounded-xl sm:rounded-2xl p-4 sm:p-5 transition-all duration-200 group hover:bg-slate-800/80 min-w-0">
                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center shrink-0 bg-[var(--accent-global-dim)] border border-[var(--accent-global)]/30 transition-all duration-300 group-hover:scale-110 group-hover:rotate-3 group-hover:shadow-lg group-hover:shadow-purple-500/20">
                  <Icon className="w-4 h-4 sm:w-5 sm:h-5 text-[var(--accent-global)]" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-[9.5px] sm:text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-0.5">{label}</div>
                  <div className="text-xs sm:text-sm font-semibold text-white truncate">{value}</div>
                </div>
                <ArrowRight className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-slate-600 transition-all duration-200 group-hover:translate-x-1 group-hover:text-[var(--accent-global)] shrink-0" />
              </a>
            ))}
            ...
            <div className="bg-gradient-to-br from-slate-900 to-slate-800 border border-white/[0.07] rounded-xl sm:rounded-2xl p-4 sm:p-5 min-w-0">
              <div className="flex items-center gap-2.5 sm:gap-3 mb-2.5 sm:mb-3">
                ...
                <div className="text-white font-bold text-xs sm:text-sm">Available Right Now</div>
              </div>
              <p className="text-[11px] sm:text-[12px] leading-relaxed text-slate-400">
              ...
              <div className="mt-3.5 sm:mt-4 pt-3.5 sm:pt-4 border-t border-white/[0.06] grid grid-cols-3 gap-2 sm:gap-3 text-center">
                {[{ v: "150+", l: "Projects" }, { v: "50+", l: "Clients" }, { v: "4.9★", l: "Rating" }].map((s) => (
                  <div key={s.l}>
                    <div className="text-white font-bold text-sm sm:text-base">{s.v}</div>
                    <div className="text-slate-500 text-[9px] sm:text-[10px] font-semibold uppercase tracking-wider">{s.l}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div className="relative w-full z-0 overflow-hidden">
        ...
        <div className="absolute top-4 sm:top-6 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2 bg-slate-950/90 backdrop-blur-sm px-3.5 py-1.5 sm:px-5 sm:py-2.5 rounded-full border border-white/[0.14] shadow-xl max-w-[92vw]">
          <MapPin className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[var(--accent-global)] shrink-0" />
          <span className="text-[11px] sm:text-xs font-bold text-white tracking-wide truncate">{COMPANY.name} — {settings.location}</span>
        </div>
        <iframe
      ...
      {/* ── MARQUEE (below map) ── */}
      <div className="relative overflow-hidden bg-slate-950 py-8 sm:py-10">
        <style>{`
        ...
        <div className="contact-marquee-wrap relative z-10 w-full overflow-hidden"
          ...
          <div className="contact-marquee-track text-[40px] sm:text-[70px] md:text-[100px] font-black tracking-tighter uppercase select-none font-sora">
            {[...Array(2)].map((_, i) => (
            ...
            {[...Array(2)].map((_, i) => (
              <span key={`b${i}`} className="inline-flex items-center gap-4 sm:gap-6 px-4 sm:px-6">
                <span className="text-white/90">GET IN</span>
                <span style={{ color: "var(--accent-global)" }}>TOUCH</span>
                <span className="text-white/20">•</span>
                <span className="text-white/90">START YOUR</span>
